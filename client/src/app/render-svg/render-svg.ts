...
import { animationDelay, cellBgColors, cellColors, cellSize, falbackCellColor, fallbackCellBgColor } from './constants'
import {
...
export class RenderSVG {
  ...
  private async renderMerge(event: TileMergeEvent): Promise<void> {
    ...
    const promise = new Promise<void>(resolve => {
      ...
      const textElement = tile.select('text').first() as any
